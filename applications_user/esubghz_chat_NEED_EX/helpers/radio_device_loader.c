...
static void radio_device_loader_power_on()
{
    ...
    if (attempts == 0)
    {
        ...
        {
            FURI_LOG_E("FjoneSubGhz", "Error power otg enable. BQ2589 check otg fault = %d", furi_hal_power_check_otg_fault() ? 1 : 0);
        }
    }
}
...
const SubGhzDevice *radio_device_loader_set(uint32_t device_ind)
{
    ...
    switch (device_ind)
    {
        case 1:
            ...
            {
                FURI_LOG_I("Fjone", "radio_device_loader_set: external is connect");
                radio_device_loader_power_on();
                device = subghz_devices_get_by_name(SUBGHZ_DEVICE_CC1101_EXT_NAME);
                break;
            }
            else{
                FURI_LOG_I("Fjone", "radio_device_loader_set: external is NOT connect");
