...
{
    bool ret = crypto_ctx_decrypt(state->crypto_ctx, state->rx_buffer, rx_size, (uint8_t *)state->rx_str_buffer);
...
void tx_msg_input(ESubGhzChatState *state)
{
    ...
    if (state->encrypted)
    {
        ...
        crypto_ctx_encrypt(state->crypto_ctx, (uint8_t *)furi_string_get_cstr(state->msg_input), msg_len, state->tx_buffer);
    }
    else
    {
        tx_size += 2;
...
{
    FURI_LOG_I("Fjone", "in enter_chat");
...
{
    FURI_LOG_I(APPLICATION_NAME, "esubghz_chat_custom_event_callback");
    furi_assert(context);
    ESubGhzChatState *state = context;
...
{
    FURI_LOG_I(APPLICATION_NAME, "esubghz_chat_navigation_event_callback");
    furi_assert(context);
    ESubGhzChatState *state = context;
...
{
    FURI_LOG_I(APPLICATION_NAME, "esubghz_hooked_draw_callback");
    ...
    FURI_LOG_I("Fjone", "esubghz_hooked_draw_callback: call ori callback");
...
{
    FURI_LOG_I(APPLICATION_NAME, "esubghz_hooked_input_callback");
    ...
    {
        FURI_LOG_I("Fjone", "back key input");
    ...
    if (event->key == InputKeyOk)
    {
         ...
         * locking */
        FURI_LOG_I("Fjone", "OK key input");
        if (state->view_dispatcher->current_view == text_box_get_view(state->chat_box) && !(state->kbd_ok_input_ongoing))
        {
            /* lock keyboard upon long press of Ok button */
            if (event->type == InputTypeLong)
            {
                FURI_LOG_I("Fjone", "OK key Input long");
                kbd_lock(state);
            }
        ...
        {
            FURI_LOG_I("Fjone", "OK key press");
            state->kbd_ok_input_ongoing = true;
        }
        else if (event->type == InputTypeRelease)
        {
            FURI_LOG_I("Fjone", "OK key release");
    ...
    if (event->key == InputKeyLeft)
    {
        ...
        if (state->view_dispatcher->current_view == text_box_get_view(state->chat_box) && !(state->kbd_left_input_ongoing))
        {
            /* go to msg input upon short press of Left button */
            if (event->type == InputTypeShort)
            {
                view_dispatcher_send_custom_event(state->view_dispatcher, ESubGhzChatEvent_GotoMsgInput);
            }
    ...
    if (event->key == InputKeyRight)
    {
        ...
        if (state->view_dispatcher->current_view == text_box_get_view(state->chat_box) && !(state->kbd_right_input_ongoing))
        {
            /* go to key display upon short press of Right button
             */
            if (event->type == InputTypeShort)
            {
                view_dispatcher_send_custom_event(state->view_dispatcher, ESubGhzChatEvent_GotoKeyDisplay);
            }
...
static void esubghz_bgloader_loop(ESubGhzChatState *state, const char *bg_app_path)
{
    while (true)
    {
        ...
        furi_check(furi_message_queue_put(bg_app->to_loader, &msg, FuriWaitForever) == FuriStatusOk);
...
int32_t esubghz_chat(const char *args)
{
    ...
    FURI_LOG_I("Fjone", "set chat name prefix done");
    ...
    view_port_draw_callback_set(state->view_dispatcher->view_port, esubghz_hooked_draw_callback, state);
    view_port_input_callback_set(state->view_dispatcher->view_port, esubghz_hooked_input_callback, state);

    FURI_LOG_I("Fjone", "set view dispatcher callback skip");

    view_dispatcher_enable_queue(state->view_dispatcher);

    FURI_LOG_I("Fjone", "View dispatcher enable queue done");

    /* set callbacks for view dispatcher */
    view_dispatcher_set_event_callback_context(state->view_dispatcher, state);

    view_dispatcher_set_custom_event_callback(state->view_dispatcher, esubghz_chat_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(state->view_dispatcher, esubghz_chat_navigation_event_callback);
    view_dispatcher_set_tick_event_callback(state->view_dispatcher, esubghz_chat_tick_event_callback, TICK_INTERVAL);
    FURI_LOG_I("Fjone", "View dispatcher set custom/navigation/tick event_callback done");

    /* add our two views to the view dispatcher */
    view_dispatcher_add_view(state->view_dispatcher, ESubGhzChatView_Menu, submenu_get_view(state->submenu));
    view_dispatcher_add_view(state->view_dispatcher, ESubGhzChatView_Input, text_input_get_view(state->text_input));
    view_dispatcher_add_view(state->view_dispatcher, ESubGhzChatView_HexKeyInput, byte_input_get_view(state->hex_key_input));
    view_dispatcher_add_view(state->view_dispatcher, ESubGhzChatView_ChatBox, text_box_get_view(state->chat_box));
    view_dispatcher_add_view(state->view_dispatcher, ESubGhzChatView_KeyDisplay, dialog_ex_get_view(state->key_display));
    view_dispatcher_add_view(state->view_dispatcher, ESubGhzChatView_NfcPopup, popup_get_view(state->nfc_popup));
    FURI_LOG_I("Fjone", "View dispatcher add view done");

    /* get the GUI record and attach the view dispatcher to the GUI */
    /* no error handling here, don't know how */
    Gui *gui = furi_record_open(RECORD_GUI);
    view_dispatcher_attach_to_gui(state->view_dispatcher, gui, ViewDispatcherTypeFullscreen);
    FURI_LOG_I("Fjone", "View dispatcher attach GUI record done");

    /* switch to the key menu scene */
    // 在这里进入了keymanu对应的菜单
    scene_manager_next_scene(state->scene_manager, ESubGhzChatScene_KeyMenu);

    FURI_LOG_I("Fjone", "switch to ESubGhzChatScene_KeyMenu done");
    ...
    FURI_LOG_I("Fjone", "bgloader loop done");
    ...
    FURI_LOG_I("Fjone", "stop the subghz worker done");
    ...
    FURI_LOG_I("Fjone", "close GUI record done");
    ...
    FURI_LOG_I("Fjone", "remove view dispatcher view");

    /* close notification record */
    furi_record_close(RECORD_NOTIFICATION);

    FURI_LOG_I("Fjone", "code notification record done");
    ...
    FURI_LOG_I("Fjone", "clear key and password done");
    ...
    FURI_LOG_I("Fjone", "deinit subghz device done");

    /* exit suppress charge mode */
    furi_hal_power_suppress_charge_exit();

    FURI_LOG_I("Fjone", "exit suppress charge mode done");
...
err_alloc:
    FURI_LOG_I("Fjone", "free everything done");
