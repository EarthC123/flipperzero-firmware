...
static void key_menu_cb(void *context, uint32_t index)
{
    ...
    switch (index)
    {
        case ESubGhzChatKeyMenuItems_NoEncryption:
            ...
            view_dispatcher_send_custom_event(state->view_dispatcher, ESubGhzChatEvent_KeyMenuNoEncryption);
            break;

        case ESubGhzChatKeyMenuItems_Password:
            view_dispatcher_send_custom_event(state->view_dispatcher, ESubGhzChatEvent_KeyMenuPassword);
            break;
...
void scene_on_enter_key_menu(void *context)
{
    ...
    submenu_add_item(state->submenu, "No encryption", ESubGhzChatKeyMenuItems_NoEncryption, key_menu_cb, state);
    submenu_add_item(state->submenu, "Password", ESubGhzChatKeyMenuItems_Password, key_menu_cb, state);
    submenu_add_item(state->submenu, "Hex Key", ESubGhzChatKeyMenuItems_HexKey, key_menu_cb, state);
    submenu_add_item(state->submenu, "Generate Key", ESubGhzChatKeyMenuItems_GenKey, key_menu_cb, state);
