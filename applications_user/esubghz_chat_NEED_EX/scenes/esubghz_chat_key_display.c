...
void key_display_result_cb(DialogExResult result, void *context)
{
    ...
    switch (result)
    {
        ...
        case DialogExResultCenter:
            if (state->encrypted)
            {
                view_dispatcher_send_custom_event(state->view_dispatcher, ESubGhzChatEvent_KeyDisplayShare);
            }
            break;
...
void scene_on_enter_key_display(void *context)
{
    ...
    if (state->encrypted)
    {
        ...
        snprintf(state->key_hex_str, KEY_HEX_STR_SIZE,
                 ...
                 "%02hX%02hX%02hX%02hX"
                 "%02hX%02hX%02hX%02hX",
                 key[0], key[1], key[2], key[3], key[4], key[5], key[6], key[7], key[8], key[9], key[10], key[11], key[12], key[13], key[14], key[15], key[16], key[17], key[18], key[19], key[20],
                 key[21], key[22], key[23], key[24], key[25], key[26], key[27], key[28], key[29], key[30], key[31]);
